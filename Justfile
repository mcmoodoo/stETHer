...
check-steth ADDRESS="0xA0c5Df94F8dd2f9aB6a4AD7A323a924670603Df8":
    @echo "Checking stETH contract at: $(jq -r '.contracts.StETH' ui/src/deployments/deployments-unichain.json)"
    @echo "Contract code size: $(cast code $(jq -r '.contracts.StETH' ui/src/deployments/deployments-unichain.json) --rpc-url http://localhost:8545 | wc -c) bytes"
    @echo "stETH balance for {{ADDRESS}}:"
    @balance_wei=$(cast call $(jq -r '.contracts.StETH' ui/src/deployments/deployments-unichain.json) "balanceOf(address)" {{ADDRESS}} --rpc-url http://localhost:8545 | cast --to-dec) && cast --from-wei $balance_wei eth
